...
import { Star, Flame, Award, Activity, X, Calendar, CheckCircle2, XCircle, Clock, CreditCard, MapPin, ArrowRight } from "lucide-react";
      ...
      <div className="bg-background rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 pt-4">
          <div className="flex gap-2 border-b border-border overflow-x-auto pb-0.5" style={{ scrollbarWidth: "thin", WebkitOverflowScrolling: "touch" }} onWheel={(e) => { if (e.deltaY !== 0) { e.currentTarget.scrollLeft += e.deltaY; } }}>
            ...
            <button onClick={() => setTab("abonnement")} className={`px-4 py-2 text-sm font-medium border-b-2 whitespace-nowrap ${tab === "abonnement" ? "border-accent text-foreground" : "border-transparent text-muted-foreground"}`}>Abonnement</button>
          </div>
        </div>
        <div className="px-6 py-6">
          {tab === "bilan" ? (
            ...
            <>
              ...
              {tab === "abonnement" && (
                <div className="space-y-4">
                  {abonnement === undefined || programmesAssignes === undefined ? (
                    <div className="flex justify-center py-12"><div className="w-8 h-8 border-4 border-secondary border-t-primary rounded-full animate-spin" /></div>
                  ) : (
                    <>
                      {abonnement ? (
                        <div className="bg-card border border-secondary/30 rounded-lg p-4">
                          <p className="text-xs font-semibold text-secondary uppercase tracking-wide flex items-center gap-1.5 mb-2"><CreditCard className="w-3.5 h-3.5" /> Abonnement actif</p>
                          <p className="text-sm font-semibold text-foreground">{abonnement.produit?.nom} — {abonnement.produit?.prix_ttc}€/mois</p>
                          {abonnement.adresse && <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1"><MapPin className="w-3 h-3" /> {abonnement.adresse}</p>}
                          {abonnement.date_fin_engagement && new Date(abonnement.date_fin_engagement) > new Date() && (
                            <p className="text-xs text-muted-foreground mt-1">Engagement jusqu'au {new Date(abonnement.date_fin_engagement).toLocaleDateString("fr-FR")}</p>
                          )}
                          {!abonnement.renouvellement_actif && <p className="text-xs text-destructive mt-1">Ne se renouvellera pas</p>}
                        </div>
                      ) : (
                        <p className="text-muted-foreground text-center py-4">Aucun abonnement actif.</p>
                      )}

                      {programmesAssignes.length > 0 ? (
                        <div className="space-y-2">
                          <p className="text-xs font-semibold text-accent uppercase tracking-wide">Programme(s) assigné(s)</p>
                          {programmesAssignes.map((a) => (
                            <Link
                              key={a.id}
                              to={`/admin/programmes?open=${a.programme_id}`}
                              className="flex items-center justify-between bg-card border border-accent/20 rounded-lg p-3 hover:border-accent transition-colors"
                            >
                              <div>
                                <p className="text-sm font-medium text-foreground">{a.programme?.name || "Programme"}</p>
                                <p className="text-xs text-muted-foreground">Depuis le {new Date(a.date_debut).toLocaleDateString("fr-FR")}</p>
                              </div>
                        ...
                        <p className="text-muted-foreground text-center py-4">Aucun programme assigné.</p>
                      )}
                    </>
                  )}
                </div>
