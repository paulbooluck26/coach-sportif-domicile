...
export default function BlocsPanel({ seanceId, onOpen }) {
  ...
  const [form, setForm] = useState({ titre: "", ordre: 0, repos_entre_exercices: 60, rounds: 1, rest_between_rounds: 60, rest_between_rounds_unit: "secondes" });
  ...
  const submit = async () => {
    ...
    setAdding(false); setForm({ titre: "", ordre: 0, repos_entre_exercices: 60, rounds: 1, rest_between_rounds: 60, rest_between_rounds_unit: "secondes" }); load();
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {!adding && <button onClick={() => { setAdding(true); setForm({ titre: "", ordre: items.length || 0, repos_entre_exercices: 60, rounds: 1, rest_between_rounds: 60, rest_between_rounds_unit: "secondes" }); }} className="bg-primary text-primary-foreground px-4 py-2 rounded-md text-sm font-semibold flex items-center gap-1.5"><Plus className="w-4 h-4" /> Bloc</button>}
      </div>
